...
final class AcceptedLanguagesLogPresenter implements LogPresenterInterface
{
    ...
    public function present($languages): string
    {
        if ($this->isEmptyData($languages)) {
            return sprintf('Retrieved no accepted languages - the list is empty [%s event].', $this->event);
        }

        return sprintf(
            'Retrieved "%s" accepted languages [%s event].',
